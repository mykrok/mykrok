...
```
--config, -c PATH    Configuration file path (default: ~/.config/strava-backup/config.toml)
...
```
strava-backup export fittrackee [OPTIONS]
...
```
strava-backup view stats [OPTIONS]
...
```
strava-backup gpx [OPTIONS] [SESSION...]
...
| `STRAVA_BACKUP_CONFIG` | Config file path (overrides default) |
| `STRAVA_BACKUP_DATA_DIR` | Data directory path |
...
Default location: `~/.config/strava-backup/config.toml`
